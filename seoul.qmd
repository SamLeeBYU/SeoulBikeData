...
create_interaction_terms = function(t){
...
seoul.bike.factors.alt = c(seoul.bike.factors, "ExtremeWind")
seoul.bike$Visibility = log(seoul$Visibility+1)
seoul.bike <- create_interaction_terms(seoul.bike)
...
seoul.bike.x = as.matrix(seoul.bike[(length(bike.deltas)+1):nrow(seoul),model.bike.factors])
seoul.bike.y = as.matrix(seoul.bike[(length(bike.deltas)+1):nrow(seoul),"RentedBikeCount"])
...
$\text{RentedBikeCount}_{st} = \beta_0 + \beta_1ln(\text{Visibility}_{st}+1) + \beta_2\text{Rainfall}_{st} + \beta_3\text{Humidity}_{st}+\beta_4\text{Temperature}_{st}+\beta_5(ln\text{WindSpeed}_{st}+1)+\beta_6I(\text{ExtremeWind}_{st}=1)+\beta_7\text{DewPointTemp}_{st}+\beta_8\text{Snowfall}_{st}+\beta_9\text{SolarRadiation}_{st}+\beta_{10}I(\text{Day}_t=\text{Holiday}_{t})\\+\beta_{11}I(\text{Season}_s = \text{Winter}) + \beta_{12}I(\text{Season}_s = \text{Summer}) + \beta_{13}I(\text{Season}_s = \text{Autumn}) + \sum_{t=1}^{23}\beta_{13+t}I(\text{Hour}_t=t) \\\sum_{p=1}^{9}\delta_p\text{RentedBikeCount}_{st-p} + \beta_{36}\text{Humidity}_{st}\text{SolarRadiation}_{st} + ... + \eta_{st}$
...
seoul.bike.factors = c(seoul.bike.factors, "ExtremeWind")
seoul$Visibility = log(seoul$Visibility+1)
seoul = create_interaction_terms(seoul)
