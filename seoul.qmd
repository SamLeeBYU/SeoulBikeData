...
seoul <- read.csv("SeoulBikeData.csv", header=F, skip=1) %>% setNames(
  ...
  mutate(
    ...
    TransformedRentedBikeCount = log(RentedBikeCount+1)
  ) %>% arrange(Time)
```
...
seoul %>% group_by(Seasons) %>%
  mutate(
    ...
    WindSpeedTransformed = ifelse(
      WindSpeed < quantile(WindSpeed, 0.9), WindSpeed, 0
    )
  ) %>% ungroup() -> seoul

#Transform Visibility
seoul$VisibilityTransformed = seoul$Visibility^(-1/2)
...
```{r}
elastic.coef = tibble(
  Rank = 1:length(seoul.elastic$coefficients),
