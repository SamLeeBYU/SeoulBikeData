...
Based on the [Residuals vs. Fitted Values plot](#a21), the linearity assumption appears to be met. There is a straight blue curve across most of the fitted values with majority of the residuals randomly distributed around the horizontal line at y = 0.

From the [Normal Q-Q plot](#a21), the plot shows that the points deviate from the reference line, especially in the tails. This suggests that the data may have heavier tails than a normal distribution, indicating the presence of outliers or a non-normal distribution of residuals.
...
Ketherine Wang: My major contributions are the analyses, results, and interpretation section, specifically diagnostic checks, confidence intervals, prediction intervals, and the corresponding interpretations of each for both models and research questions. We worked on the code for setting up the preliminary models together, and I helped with the transformations of the preliminary models in. I also helped to craft the abstract, problem and motivation, and conclusion paragraphs.

Sam Lee: I organized appendix and formalized the time series regression models. I wrangled the data and and created the iterative processes to include all necessary interaction terms for each model. I used elastic regression to determine which coefficients were appropriate for each model. I also gathered current weather data to run prediction intervals on our model to predict $\Delta\text{Temp}_{st}$.
...
NOTE: Not all parameter coefficients are estimated to be > 0. See model output to see which coefficients.
